...
// components
import TableDropdown from "components/Dropdowns/TableSizeDropdown.js";
export default function CardTable({ color }) {

  const {showModal} = useContext(modalContext);

  const query = useGetSizes();
  if(query.isLoading){
    return(
      <>
        <div>Cargando....</div>
      ...
      >
              ...
              >
                ...
                <button 
                  ...
                  onClick={()=>{
                    ...
                    showModal(null,'size-modal')
                  }}>
                  <i className="fas fa-plus"></i> Añadir Talla
                </button>
              </h3>
            </div>
          </div>
        ...
        <div className="block w-full overflow-x-auto">
          ...
          <table className="items-center w-full bg-transparent border-collapse">
            ...
            <tbody>
              {query.data.data&&query.data.data.sizes.map((size)=>{
