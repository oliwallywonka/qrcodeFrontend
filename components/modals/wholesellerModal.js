...
import {usePostWholeseller,usePutWholeseller} from '../../hooks/useWholeseller'

export default function Modal() {

  const validate = Yup.object({
    phone: Yup.number()
      .min(8,'El telefono introducido es invalido')
      .required('El telefono es obligatorio'),
    name: Yup.string()
      .min(4,'Nombre es muy corto')
      .required('La Nombre es obligatoria'),
  });

  const {hideModal,visible,payload} = useContext(modalContext);
  const [handlePostRequest] = usePostWholeseller();
  const [handlePutRequest] = usePutWholeseller();
